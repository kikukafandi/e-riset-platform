...
@section('title', 'Disposisi Eselon III')
@section('content')
<div class="container mt-4">
    <h1 class="mb-4">Disposisi Dokumen - Eselon III</h1>

    <div class="card">
        <div class="card-header">
            <h5 class="mb-0">Dokumen dari Eselon IV</h5>
        </div>
        <div class="card-body">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($doc->admin_validation_message)
                                            <span class="text-muted">{{ Str::limit($doc->admin_validation_message, 30) }}</span>
                                        @else
                                            <span class="text-muted">-</span>
                                        @endif
                                    </td>
                                    <td>
                                        <button class="btn btn-primary btn-sm" onclick="forwardDocument({{ $doc->id }}, 'eselon_ii')">
                                            <i class="fas fa-forward"></i> Teruskan ke Eselon II
                                        </button>
            ...
            @else
                <div class="text-center py-4">
                    ...
                    <p class="text-muted">Dokumen yang diteruskan dari Eselon IV akan muncul di sini.</p>
                </div>
            @endif
        </div>
    </div>
            ...
            <div class="modal-body">
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="forwardNotes" rows="3" 
                              placeholder="Tambahkan catatan untuk Eselon II..."></textarea>
                </div>
            </div>
...
function approveDocument(docId) {
    if (confirm('Yakin ingin menyetujui dokumen ini langsung?')) {
        forwardDocumentRequest(docId, 'final_approval', 'Disetujui langsung oleh Eselon III');
    }
}
